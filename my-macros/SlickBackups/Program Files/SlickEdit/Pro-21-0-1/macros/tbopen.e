...
#define TBOPEN_FORM '_tbopen_form'
...
#define CHANGING_EXPLORER_TREE         _new_file_button.p_user
...
#define timer_refresh_flags            _file_tree.p_user
...
{
   int form_wid=_tbGetActiveOpenForm();
   if (!form_wid) {
      return 0;
   }
   _nocheck _control _file_tree;
...
static void maybeRefreshFilesOnDisk(){
   ...
   treeWid.timer_refresh_flags |= OTBFO_FILE_ON_DISK;
...
static void maybeRefreshSameDirFiles(){
   ...
   treeWid.timer_refresh_flags |= OTBFO_SAME_DIR_FILE;
   if(force_timer_delay == false) {
      startRefreshTimer(treeWid.p_active_form);
   }
}
...
static void maybeRefreshOpenFiles(_str fileClosedPath = ''){
    ...
    foreach (i => v in gtbOpenFormList) {
       ...
       if ((def_opentb_view_flags & OTBFO_OPEN_FILE)) {
          ...
          treeWid.timer_refresh_flags |= OTBFO_OPEN_FILE;
          if (force_timer_delay == false) {
              startRefreshTimer(treeWid.p_active_form);
          }
       }
...
static void maybeRefreshHistoryFiles(){
   ...
   treeWid.timer_refresh_flags |= OTBFO_HISTORY_FILE;
   if(force_timer_delay == false) {
      startRefreshTimer(treeWid.p_active_form);
   }
}
...
static void _maybeRefreshWorkspaceFilesForForm()
{
   ...
   treeWid.timer_refresh_flags |= OTBFO_PROJECT_FILE;
   if(force_timer_delay == false) {
#if 0
...
static void setExplorerTreePath(_str newPath=null, 
                                int doRefresh=0, 
                                boolean show_dotfiles=doShowHiddenFiles(), 
                                boolean doCd=(def_opentb_flags & OPENTB_SYNC_DIRECTORY) != 0,
                                boolean expandAllSiblings=(def_opentb_flags & OPENTB_EXPAND_ALL_SIBLINGS) != 0)
{
...
void checkViews()
{
   if (def_opentb_view_flags != CURRENT_VIEWS) {
      ...
      if (curSameDir && !shouldSameDir) {
         ...
         // add these
         rescanSameDirFiles();
         CURRENT_VIEWS |= OTBFO_SAME_DIR_FILE;
         updateSort = true;
      }
 ...
 */
static void refreshOpenToolWindows(boolean checkForWorkspaceRefresh = false)
{

   TBOPEN_FORM_INFO v;
   int i;
   boolean do_workspace_refresh=false;
   foreach (i => v in gtbOpenFormList) {
      i.rescanFilesOnDisk();
...
static _str getFilePath(int treeIndex)
{
   ...
   if (path=='') {
      ...
      if (bmpIdx1== _pic_otb_file_proj ||
          bmpIdx1==_pic_otb_file_proj_open ||
          bmpIdx1==_pic_otb_file_wksp ||
          bmpIdx1==_pic_otb_file_wksp_open
          ) {
         path='.';
      }
   }
...
void _tbopen_form.on_resize()
{
   ...
   boolean first_resize = 0 == (p_window_flags & VSWFLAG_ON_RESIZE_ALREADY_CALLED);

   if ( first_resize ) {
      xpos := _grabbar_vert._retrieve_value();
      if ( xpos != null && isinteger(xpos) ) {
         _grabbar_vert.p_x = xpos;
      }
      ypos := _grabbar_horz._retrieve_value();
 ...
 */
void _prjopen_opentb(boolean singleFileProject)
{
 ...
 */
void _prjclose_opentb(boolean singleFileProject)
{
   if (singleFileProject) return;
...
static void showRightClickMenu()
{
   ...
   if (numSelected == 1) {
      ...
      isDirOnly = treeWid.isSelectedFileTreeItemDirectory(index);
      _str selDir = treeWid._TreeGetCaption(index);
...
static void copyPathToClipboard(boolean namesOnly=false){
