...
static void autorestore_layout(_str option, _str info='', _str restoreName='', boolean restoreOnlyFloating=false)
{
   tw_sanity();
   ...
   say(" xx " :+ option :+ " x1 " :+ info :+ " x2 " :+ restoreName);
   ...
   */
   ...
   if ( option == 'r' || option == 'n' ) {
      ...
      if( isinteger(nRestoreToolWindow) && nRestoreToolWindow > 0 ) {
         ...
         boolean dockingAllowed = tw_is_docking_allowed();
      ...
      // Tool-window info
      int nRestoreToolWindowInfo = g_toolwindowtab._length();
...
static int autorestore_layout_view(_str view_name, int& view_id, boolean isCurrentLayout,
...
int _sr_fullscreen_layout(_str option='', _str info='')
{
   boolean isCurrent = !_tbDebugQMode() && _tbFullScreenQMode();
...
int _sr_standard_layout(_str option='', _str info='')
{
   boolean isCurrent = !_tbDebugQMode() && !_tbFullScreenQMode();
...
int _sr_debug_layout(_str option='', _str info='')
{
   boolean isCurrent = _tbDebugQMode() && !_tbFullScreenQMode() && !_tbDebugQSlickCMode();
...
int _sr_fullscreen_debug_layout(_str option='', _str info='')
{
   boolean isCurrent = _tbDebugQMode() && _tbFullScreenQMode() && !_tbDebugQSlickCMode();
...
int _sr_slickc_debug_layout(_str option='', _str info='')
{
   boolean isCurrent = _tbDebugQMode() && !_tbFullScreenQMode() && _tbDebugQSlickCMode();
...
int _sr_fullscreen_slickc_debug_layout(_str option='', _str info='')
{
   boolean isCurrent = _tbDebugQMode() && _tbFullScreenQMode() && _tbDebugQSlickCMode();
...
static void _autorestore_from_view(int temp_view_id, boolean restoreWindowState)
{
   int focus_wid=0;
   if (_isUnix()) {
      focus_wid = _get_focus();
...
int _sr_app_layout(_str option='', _str info='', _str restoreFromInvocation='', _str relativeToDir=null)
{
   //return 0;
   option = lowcase(option);
   if ( option == 'r' || option == 'n' ) {

      boolean restoreOnlyFloating = restoreFromInvocation == '0';
      autorestore_layout(option, info, 'APP_LAYOUT', restoreOnlyFloating);
   } else {
...
void debug_switch_mode(boolean onoff, boolean slickc=false)
{
   if ( !isinteger(onoff) ) {
      onoff = !_tbDebugQMode();
   }
   boolean restoreWindowState = _tbFullScreenQMode();
   if ( onoff ) {
      if ( _tbDebugQMode() ) {
...
int _MDIGetActiveMDIChild()
{
   ...
   if ( !child_wid ) {
      boolean mdi_with_cmdline = _MDIWindowHasMDIArea(mdi_wid);
      if ( mdi_with_cmdline ) {
         return 0;
      }
