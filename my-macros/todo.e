...
#define _TBTODO      "ToDo_form"
// Menu name
#define _TBTODO_MENU       "ToDoMenu"
...
static void RenderTree()
{
   ...
   // Clear the tree
   todo_tree._TreeDelete(root, "C");

   foreach (auto ps_project in projects) {
      if (settings.sort == SB_PROJECT) {
         proj = todo_tree._TreeAddItem(root, ps_project.friendly_name, TREE_ADD_AS_CHILD,
      ...
      foreach (auto fs_file in ps_project.files) {
         ...
         file = todo_tree._TreeAddItem(proj, fs_file.friendly_name, TREE_ADD_AS_CHILD, 
         ...
         todo_tree._TreeSetUserInfo(file, temp);
...
{
   tw_register_form('ToDo_form', 0, DOCKAREAPOS_NONE);
}
 ...
 */
void _on_unload_module_ToDo_form(_str filename)
{
   if (filename == _TBTODO_FILE :+ "x") {
      int wid = tw_is_visible('ToDo_form');
      if (wid)
...
void _tbSaveState_ToDo_form(TODO_SETTINGS& state, boolean closing)
{
   todo_tree._TreeSaveNodes(settings.node_state);
   state = settings;
}

void _tbRestoreState_ToDo_form(TODO_SETTINGS& state, boolean opening)
{
...
_command void activate_todo()  name_info(','VSARG2_EDITORCTL)
{
   int treewid = _tbGetWid(_TBTODO);
   activate_tool_window('ToDo_form',true,'todo_tree');
   if (treewid <= 0)
      ToDo_GenerateList();
}
...
_command void ToDo_expand_all() name_info(',')
{
   int treewid = _tbGetWid(_TBTODO);
   if (treewid == 0)
      activate_todo();
   ...
   treewid.todo_tree._TreeExpandAll();
}

_command void ToDo_collapse_all() name_info(',')
{
   int treewid = _tbGetWid(_TBTODO);
   if (treewid == 0)
      activate_todo();
   ...
   treewid.todo_tree._TreeCollapseAll();
}




defeventtab ToDo_form;
...
void todo_tree.on_highlight(int index, _str caption="")
{
   if (index <= 0) {
      return;
   }
   ...
   cm.file_name = info.filename;
   cm.line_no = info.line;
   cm.column_no = info.column;
   cb_refresh_output_tab(cm, true, true, false, 0);
   p_window_id = orig_wid;
}


void todo_tree.on_change(int reason, int index)
{
