                                   ...
                                   VS_TAG_RETURN_TYPE (&visited):[], int depth=0)
{
   ...
   if (pas_return_flags & VSCODEHELP_RETURN_TYPE_GLOBALS_ONLY) {
      ...
      if (search_class_name!="" && num_matches==0) {
         tag_list_symbols_in_context(symbol, search_class_name, 0, 0, tag_files, "",
                                     num_matches, def_tag_max_function_help_protos,
                                     pushtag_mask, VS_TAGCONTEXT_ALLOW_locals|VS_TAGCONTEXT_FIND_all|VS_TAGCONTEXT_ONLY_context|VS_TAGCONTEXT_ALLOW_private|VS_TAGCONTEXT_ALLOW_protected|VS_TAGCONTEXT_ALLOW_package,
                                     true, false, visited, depth);
      }
                                    ...
                                    VS_TAG_RETURN_TYPE (&visited):[], int depth=0)
{
   ...
   for (i:=1; i<=num_matches; i++) {
      ...
      if (tag_tree_type_is_package(proc_cm.type_name) || tag_tree_type_is_class(proc_cm.type_name) || proc_cm.type_name=="enum") {
         if (proc_cm.return_type != null && proc_cm.return_type != "") {
            ...
            proc_cm.return_type = tag_join_class_name(proc_cm.member_name, proc_cm.class_name, tag_files, true, false); 
         } else {
   ...
   for (i=0; i<matchlist._length(); i++) {
      ...
      if (tag_tree_type_is_package(proc_cm.type_name)) {
         ...
         status = _pas_parse_return_type(errorArgs, tag_files, 
                                         proc_cm.member_name, proc_cm.class_name,
         ...
         if (search_class_name == "" || search_class_name == cur_class_name) {
            ...
            while (outer_class_name != "") {
               ...
               if (local_matches > 0) {
                  ...
                  rt.return_type = tag_join_class_name(rt.return_type, relInfo.class_name, tag_files, false, true);
   ...
   for (;;--stack_top) {
      ...
      if (!status) {
         ...
         for (i=0; i<num_unique; i++) {
            ...
            if (num_unique > 1) {
               if (file_eq(cm.file_name,p_buf_name) && cm.line_no:==p_line) {
                  continue;
               }
         ...
         if (match_list._length()>0) {
            ...
            for (i=0; i<match_list._length(); i++) {
               ...
               parse match_list[i] with match_tag_name "\t" match_type_name "\t" auto signature "\t" auto return_type "\t" imatch "\t" duplist;
               ...
               FunctionHelp_list[k].tagList[0].taginfo=tag_compose_tag_browse_info(cm);
               int z1;
               _str z;
               for (z1=1;;) {
                  ...
                  if (z!=imatch) {
                     ...
                     FunctionHelp_list[k].tagList[z1].filename=cm.file_name;
                     FunctionHelp_list[k].tagList[z1].linenum=cm.line_no;
                     FunctionHelp_list[k].tagList[z1].comment_flags=0;
                     FunctionHelp_list[k].tagList[z1].comments=null;
                     FunctionHelp_list[k].tagList[z1].taginfo=tag_compose_tag_browse_info(cm);
                     ++z1;
                  }
               }
               //++base_length;
...
int _pas_MaybeBuildTagFile(int &tfindex, bool withRefs=false, bool useThread=false)
{
   if (!_haveContextTagging()) {
      return VSRC_FEATURE_REQUIRES_PRO_EDITION;
   }
