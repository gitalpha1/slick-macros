...
definit()
{
   ...
   if ( arg(1)=='L' ) {
      ...
      load_picture(-1, '_f_arrow_left.ico');
      load_picture(-1, '_f_arrow_right.ico');
...
void mini_find_update_options(_str search_string, _str replace_string, _str search_options, int search_range, int mfflags, _str misc_options)
{
...
{  
   ...
   typeless value = _retrieve_value("_document_search_form.p_width");
   if (value != '') {
...
static void _ds_exit(int form_wid)
{
   ...
   _ds_update_history();
   _save_form_response();
   _append_retrieve(0, p_active_form.p_width, "_document_search_form.p_width");
   if (editorctl_wid && _iswindow_valid(editorctl_wid) && editorctl_wid._isEditorCtl()) {
      _ds_dismiss_tooltip(editorctl_wid);
      editorctl_wid._RemoveEventtab(defeventtab _editorctl_resize_helper);
      activate_window(editorctl_wid);
...
static void _init_search_flags(int search_flags)
{
   ...
   re_flags := (search_flags & (VSSEARCHFLAG_RE|VSSEARCHFLAG_PERLRE|VSSEARCHFLAG_WILDCARDRE));
   ...
   s_params.m_options = search_flags & (VSSEARCHFLAG_REVERSE|VSSEARCHFLAG_HIDDEN_TEXT|VSSEARCHFLAG_REPLACEHILIGHT);
...
static void _init_misc_search_opts(_str options)
{
   ...
   if (opt == 'mini') {
      ...
      s_params.m_options &= ~(VSSEARCHFLAG_WRAP|VSSEARCHFLAG_PROMPT_WRAP|VSSEARCHFLAG_REVERSE|VSSEARCHFLAG_HIDDEN_TEXT|VSSEARCHFLAG_REPLACEHILIGHT);
      s_params.m_listreplace = false;
      s_params.m_grep_ab = false;
...
_command void docsearch_find_menu(_str cmdline = '') name_info(',' VSARG2_EXECUTE_FROM_MENU_ONLY)
{
   ...
   case "find-in-files":
      find_in_files();
      tool_find_update_options(search_string, replace_string, search_options, -1, mfflags, misc_options);
      return;
...
static void _do_gui_replace()
{
   ...
   activate_window(editorctl_wid);
   search_options = _ds_begin_mark(search_options, search_range, auto mark_info);
...
static void _ds_translate_command(_str keyname)
{
   ...
   case "tool-gui-find":
      tool_gui_find();
      tool_find_update_options(search_string, replace_string, search_options, search_range, mfflags, misc_options);
      return;
   case "gui-replace":
   case "gui-replace-backward":
   case "gui-replace-regex":
...
{
   _str search_options = '';
   if (!ctl_matchcase.p_value) search_options = search_options:+'I';
   if (ctl_matchword.p_value) search_options = search_options:+'W';
   if (ctl_regex.p_value) {
      re_flags := P_USER_MINIFIND_RE();
      if (re_flags & VSSEARCHFLAG_PERLRE) {
         search_options = search_options:+'L';
   ...
   if (ctl_matchcolor.p_enabled && ctl_matchcolor.p_value) {
      search_options = search_options:+_findcoloroptions.p_text;
   }
...
static _str _ds_begin_mark(_str search_options, int search_range, DSMark& info)
{
...
static bool _ds_check_start_mark(int search_range)
{
...
static int doNext(bool doPrev=false, bool doReplaceNext=false, bool closeOnExit=false)
{
   ...
   if (!status) {
      ...
      if (replace_mode) {
         if (_ds_check_start_mark(search_range)) {
            replace_wrap = true;
         }
      }
   } else {
...
static void doReplace(bool doAll=false, bool doPreview=false)
{
   ...
   if (doAll) {
      ...
      if (!status) {
         msg := "Replace";
         msg = msg:+' "':+search_string:+'", "':+replace_string:+'"';
         if (search_options:!= '') {
            msg = msg:+', ':+_get_search_options_label(search_options);
         }
         if (search_range != VSSEARCHRANGE_CURRENT_BUFFER) {
            msg = msg:+', ':+_get_search_range_label(search_range);
         }
         message(msg);
      } else {
