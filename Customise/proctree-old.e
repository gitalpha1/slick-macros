...
{
   bForceLineNumberSort := _are_statements_supported(getFileTreeLangId(ParentIndex)) && (Options&PROC_TREE_STATEMENTS) && _haveContextTagging();
...
static void resizeProcs()
{
   ...
   if (!isEclipsePlugin()) {
      ...
      if (child_wid) {
         ...
         _str BufName = child_wid._GetDocumentName();
         _str CaptionName=ctlcurpath._ShrinkFilename(stranslate(BufName,"&&","&"),_proc_tree.p_width);
         if (CaptionName!=ctlcurpath.p_caption) {
            ctlcurpath.p_caption=CaptionName;
         }
                               ...
                               bool ForceUpdate=false)
{
   ...
   _str filename=_strip_filename(child_wid._GetDocumentName(),'P');
   ...
   doStatementTagging := (def_proc_tree_options&PROC_TREE_STATEMENTS) && _haveContextTagging();
   ...
   if( findCurrentProc && _TreeGetFirstChildIndex(index)>=0 ) {
      ...
      int EditorLN = child_wid.p_RLine;
                            ...
                            _str &proc_name, _str &path, int &LineNumber, int index,int f)
{
   ...
   if (child_wid) {
      if( (def_proc_tree_options&PROC_TREE_STATEMENTS) && _haveContextTagging() ) {
...
void _proc_tree.rbutton_up()
{
   ...
   if ( (def_proc_tree_options&PROC_TREE_STATEMENTS) && _are_statements_supported(lang) && _haveContextTagging()) {
...
_command void ProcTreeRunMenu() name_info(','VSARG2_CMDLINE)
{
   ...
   _str filename=_strip_filename(child_wid._GetDocumentName(),'P');
   _str path=child_wid._GetDocumentName();
