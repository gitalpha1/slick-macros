...
#define  XRETRACE_DATA_PATH   XRETRACE_PATH :+ 'xretrace-data' :+ FILESEP

#define USE_SUBFOLDER_FOR_XRETRACE_DATA
...
static boolean  xretrace_data_for_file_needs_saving;

#ifdef USE_SUBFOLDER_FOR_XRETRACE_DATA
// path1 uses a sub-folder of the current buffer - in which case there are 
// no duplicated filenames
_str get_xretrace_data_path1(_str curbuf_name)
{
   // use an xretrace sub-folder
   return strip_filename(curbuf_name, 'N') :+ FILESEP :+ 'xretrace' :+ FILESEP;
}

// path2 uses a common folder for all files, in which case there may be duplicated filenames
_str get_xretrace_data_path2(_str curbuf_name)
{
   return XRETRACE_DATA_PATH;
...
static void update_retrace_cursor_list(boolean force_update = false)
{
   ...
   if (retrace_region_line_marker_set && 
      ...
      xretrace_data_for_file_needs_saving = true;
...
static void save_retrace_data_for_file(_str fn)
{
   ...
   xretrace_data_for_file_needs_saving = false;
   ...
   if ( file_exists(maybe_quote_filename(*active))) {
...
static void open_data_file(_str name2)
{
   int new_wid, orig_wid;
...
static void buffer_switch_setup_buffer_retrace_lists()
{
   ...
   if ( !active ) {
      ...
      _str xpath1 = get_xretrace_data_path1(retrace_current_buf_name);
      _str name2 = xpath1 :+ FILESEP :+ name1 :+ '.xr';
      if ( !file_exists(_maybe_quote_filename(name2)) ) {
         // create and open
         if (make_path(xpath1) != 0)
         {
            // path1 failed, try path2
            _str xpath2 = get_xretrace_data_path2(retrace_current_buf_name);
            name2 = xpath2 :+ FILESEP :+ name1 :+ '.xr';
            if ( file_exists(_maybe_quote_filename(name2)) ) {
               open_data_file(name2);
               return;
            }
            if (make_path(xpath2) != 0)
               return;
         }
...
static void maintain_cursor_retrace_history()
{
   ...
   if ((_mdi.p_child.p_buf_id != retrace_current_buf_id) || 
      ...
      if (!retrace_ignore_current_buffer) {
         ...
         if ( xretrace_data_for_file_needs_saving ) {
...
_command void show_xbar() name_info(',')
{
   execute('show_tool_window -current-mdi xbar1');
}
